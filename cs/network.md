...
`netstat`는 접속되어 있는 서비스들의 네트워크 상태를 표시한다. 네트워크 접속, 라우팅 테이블, 네트워크 프로토콜 등의 리스트를 보여준다. 포트를 확인할때 사용한다.

`nslookup`은 DNS 관련 내용을 확인하기 위해 쓰는 명령어로 특정 도메인에 매핑된 IP를 확인하기 위해 사용한다.

`tracert`는 윈도우, `traceroute`는 리눅스에서 사용한다. 목적지 노드까지 네트워크 경로를 확인할때 사용한다. 어느 구간에서 느려지는지 확인할 수 있다. tracert www.google.com 명령어로 구글 사이트에 도달하기까지의 경로 추적을 할 수 있다. 

`ftp`를 통해 대형 파일을 전송하여 테스팅하고, `tcpdump`로 노드로 오고가는 패킷을 캡처한다. 네트워크 분석 프로그램으로 wireshark, netmon이 있다.

`네트워크 프로토콜`은 다른 장치끼리 데이터를 주고받기 위해 설정된 공통된 인터페이스이다. IEEE 또는 IETF라는 표준화 단체가 정한다. `IEEE802.3`은 유선 LAN 프로토콜로, 이를 통해 만든 기업이 다른 장치라도 서로 데이터를 수신할 수 있다. `HTTP`도 서로 약속된 인터페이스인 HTTP 프로토콜을 통해 노드들이 웹 서비스를 기반으로 데이터를 주고받는다.

</details>

<details>
  <summary>TCP/IP 4계층 모델</summary>
</details>
